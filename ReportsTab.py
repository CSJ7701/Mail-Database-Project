...
class Reports(Screen):
    def __init__(self, main_frame, ParentGUI):
        ...
        # Create frames for report options and plotting
        ...
        # Create widgets for selecting report parameters
        ...
        # List to choose which options to exclude
        ...
        # Uses the 'companies' list to create a scrollable list with each entry
        ...
        # Uses the list generated by 'self.GetYears' to fill a list with 5 years on each side of the current year.
    ...
    def CheckDate(self, event):
        """Check the validity of the entered dates."""
    ...
    def PlotAllTime(self):
        """Plot packages received over all time."""
    ...
    def PlotWeekDay(self):
        """Plot packages received and picked up by weekday."""
        excluded_names=self.GetExcludes()
        date_range=self.GetDateFilter()
        if self.report_ret_choice.get()==1:
            if excluded_names:
    ...
    def PlotTime(self):
        """Plot packages picked up by hour."""
        excluded_names=self.GetExcludes()
        date_range=self.GetDateFilter()
        if self.report_ret_choice.get()==1:
            if excluded_names:
    ...
    def PlotCompany(self):
        """Plot packages received and picked up by company."""
        excluded_names=self.GetExcludes()
        date_range=self.GetDateFilter()
        if self.report_ret_choice.get()==1:
            if excluded_names:
    ...
    def PlotGradYear(self):
        """Plot packages received by graduation year."""
        excluded_names=self.GetExcludes()
        date_range=self.GetDateFilter()
        if self.report_ret_choice.get()==1:
            if excluded_names:
